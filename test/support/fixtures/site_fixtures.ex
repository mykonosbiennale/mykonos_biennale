defmodule MykonosBiennale.SiteFixtures do
  ...
  def page_fixture(attrs \\ %{}) do
    ...
    {:ok, page} =
      Site.create_page(%{
        ...
        slug:
          Map.get(attrs, "slug") || Map.get(attrs, :slug, "test-page-#{System.unique_integer()}"),
  ...
  def section_fixture(attrs \\ %{}) do
    ...
    page =
      Map.get_lazy(attrs, "page", fn -> Map.get_lazy(attrs, :page, fn -> page_fixture() end) end)
    ...
    {:ok, section} =
      Site.create_section(%{
        ...
        slug:
          Map.get(attrs, "slug") ||
            Map.get(attrs, :slug, "test-section-#{System.unique_integer()}"),
        content:
          Map.get(attrs, "content") || Map.get(attrs, :content, "<p>Test section content</p>"),
        visible: Map.get(attrs, "visible") || Map.get(attrs, :visible, true),
        template: Map.get(attrs, "template") || Map.get(attrs, :template, "default"),
        position: Map.get(attrs, "position") || Map.get(attrs, :position, 0),
        page_id: page.id
